...
require_once('class.soap_server.php');*/
	...
	class nusoap_base
	{
		 ...
		 * @param mixed  $headers       optional string of XML with SOAP header content, or array of soapval objects for SOAP headers, or associative
		 *                              array
			...
			elseif($authtype == 'digest')
			{
				if(isset($digestRequest['nonce']))
				{
					...
					$this->setHeader('Authorization',
					                 'Digest username="' . $username . '", realm="' . $digestRequest['realm'] . '", nonce="' . $nonce . '", uri="' .
					                 $this->digest_uri . $opaque . '", cnonce="' . $cnonce . '", nc=' . sprintf("%08x", $digestRequest['nc']) .
	...
	class nusoap_server extends nusoap_base
	{
		                       ...
		                       $transport = 'http://schemas.xmlsoap.org/soap/http', $schemaTargetNamespace = FALSE)
		{
			...
			$this->wsdl->schemas[$schemaTargetNamespace][0]->imports['http://schemas.xmlsoap.org/soap/encoding/'][0] = array('location' => '',
			                                                                                                                 'loaded'   => TRUE);
			$this->wsdl->schemas[$schemaTargetNamespace][0]->imports['http://schemas.xmlsoap.org/wsdl/'][0]          = array('location' => '',
			                                                                                                                 'loaded'   => TRUE);
...
?><?php
	...
	class wsdl extends nusoap_base
	{
			...
			if(isset($typeDef['elements']) && is_array($typeDef['elements']))
			{
				...
				foreach($typeDef['elements'] as $eName => $attrs)
				{
					...
					else
					{
						...
						if(isset($attrs['maxOccurs']) && ($attrs['maxOccurs'] == 'unbounded' || $attrs['maxOccurs'] > 1) && isset($v) &&
		 ...
		 * @param string $style         (rpc|document) optional The style for the operation Note: when 'document' is specified, parameter and return
		 *                              wrappers are created for you automatically
		 ...
		 * @access public
		 */
		function addOperation($name, $in = FALSE, $out = FALSE, $namespace = FALSE, $soapaction = FALSE, $style = 'rpc', $use = 'encoded',
		                      $documentation = '', $encodingStyle = '')
		{
			if($use == 'encoded' && $encodingStyle == '')
			{
			...
			if(!empty($xml))
			{
				...
				if($pos_xml !== FALSE)
				{
					...
					if(preg_match("/encoding=[\"']([^\"']*)[\"']/", $xml_decl, $res))
					{
						...
						{
							$err = "Charset from HTTP Content-Type '" . $encoding . "' does not match encoding from XML declaration '" .
							       $xml_encoding . "'";
							$this->debug($err);
...
?><?php
	 ...
	 */
		 ...
		 *
		 * @return    mixed    response from SOAP call, normally an associative array mirroring the structure of the XML response, false for certain
		 *                     fatal errors
		 * @access   public
		 */
		function call($operation, $params = array(), $namespace = 'http://tempuri.org', $soapAction = '', $headers = FALSE, $rpcParams = NULL,
		...
		function loadWSDL()
		{
			...
			$this->wsdl = new wsdl('', $this->proxyhost, $this->proxyport, $this->proxyusername, $this->proxypassword, $this->timeout,
		 ...
		 * @param    string $password
		 * @param    string $authtype    (basic|digest|certificate|ntlm)
		 * @param    array  $certRequest (keys must be cainfofile (optional), sslcertfile, sslkeyfile, passphrase, verifypeer (optional), verifyhost
